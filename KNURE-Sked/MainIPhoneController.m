...
- (void)drawMainView {
    ...
    for(NSDate *lessonDate in dateList) {
        date = [[UILabel alloc]initWithFrame:CGRectMake(dayShift + 55, 5, 110, 20)];
        date.text = [timer.dayMonthWeekDay stringFromDate:lessonDate];
        date.textAlignment = NSTextAlignmentCenter;
        [date setFont:[UIFont fontWithName: @"HelveticaNeue-Light" size: 16.0f]];
        ...
        while ([[timer.dayMonthYear stringFromDate:startLessonDate] isEqualToString:
                [timer.dayMonthYear stringFromDate:lessonDate]]) {
            if (eventIndex + 1 >= eventHandler.events.count) {
                ...
                if(numberPair >= 7) { maxContentSize = 55 * 8.5; }
                lessonShift += (numberPair - 1) * 55;
                goto m1;
            }
            ...
            if (numberPair == [[[eventHandler.events objectAtIndex:eventIndex + 1]valueForKey:@"number_pair"]intValue] &&
                ...
                UIScrollView *buildInView = [[UIScrollView alloc]initWithFrame:CGRectMake(dayShift + 55, lessonShift + 5, 110, 50)];
                ...
                while (numberPair == [[[eventHandler.events objectAtIndex:eventIndex]valueForKey:@"number_pair"]intValue]) {
                    ...
                    skedCell = [[UIView alloc]initWithFrame:CGRectMake(0, 0 + innerYPosition, 110, 50)];
...
- (void)tapOnCell:(UIGestureRecognizer *)recogniser {
    ...
    ModalViewController *modalVC = [[ModalViewController alloc]initWithDictionary:eventHandler.schedule index:skedCell.tag isTablet:NO];
...
- (void)toggleMenu {
    ...
    remenuScroller = [[UIScrollView alloc]initWithFrame:CGRectMake(0,
...
#pragma mark - анимирующаяся полоска
...
- (void)initAnimation {
    ...
    [UIView animateWithDuration:animationDuration
        ...
        [timeLineIndicator setFrame:CGRectMake(0, 466, 47, 2)];
    } completion:nil];
    ...
    [UIView animateWithDuration:animationDuration
                        ...
                        options:(UIViewAnimationOptionCurveLinear |
                                 ...
                                 UIViewAnimationOptionAllowUserInteraction |
                                 UIViewAnimationOptionBeginFromCurrentState) animations:^{
                            [skedLineIndicator setFrame:CGRectMake(xPosition + 50.0f, 466, 115, 2)];
                        } completion:nil];
...
- (double)getYPosition {
    /*
     ...
     пропорция 50400 -> 100%;
     seconds -> x%
     нижняя координата по Y = 466, верхняя 27, значит шкала времени всего 439
     439 -> 100%, 4.39 -> 1%
     27 получается сдвиг
     */
...
- (void)timeUpdate {
    ...
    //TODO: после истечения времени таймер должен быть переиницализирован
}
